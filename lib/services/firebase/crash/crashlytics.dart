...
import 'package:world_time/services/package_info/package_info.dart';
import 'package:world_time/services/route_logger/route_logger.dart';
part '_exts.dart';
...
class EnhancedCrashlytics {
  ...
  Future<void> init() async {
    try {
      ...
      'Error in Enhanced Crashlytics init: $e'.appLog(level: AppLogLevel.error);
  ...
  Future<void> _setAppVersionInfo() async {
    await Future.wait([
      _firebaseCrashlytics.setCustomKey('app_version', DeviceInfo.version),
      _firebaseCrashlytics.setCustomKey(
        'build_number',
        DeviceInfo.buildNo.toString(),
      ),
      _firebaseCrashlytics.setCustomKey('package_name', DeviceInfo.packageName),
    ]);
  }
